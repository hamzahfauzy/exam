...
class KuisController
{
    ...
    function notificationParticipant()
    {
        ...
        foreach($participants as $participant)
        {
            ...
            $message = "Info ujian ".$nama.", Link: s.id/eCQGX, Email: ".$email.", Sandi: ".$password.", masuk untuk melihat jadwal ujian";
    ...
    function notificationParticipantMail()
    {
        $request = request()->post();
        $participants = $this->getParticipant($request->id);
        $response = [];
        ...
        foreach($participants as $participant)
        {
            ...
            $send = $mail->send($request->user_email,"Info Ujian PT. Kawasan Industri Nusantara",$message);
            if($send != 1)
                $response[] = ['status' => false, 'message' => $send];
            else
                $response[] = ['status' => 'success', 'message' => $send];
        }
